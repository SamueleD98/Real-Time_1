...
//Declaration of the three mutexes attributes and setting the priority ceiling protocol
//The first two mutexes have the same priority ceiling (both of them are associated to task1 => highest priority)
//so they both relate to attr1, meanwhile the third mutex relate to attr2
...
main()
{
    ...
    //----------------------------------------------------------------------------------------------------------------------------------------------
    	...
    	//Assign to the mutexes the prio ceiling from parameters[i].sched_priority 
    	//Two mutexes will have as prio ceiling the first task priority, the third will have the second task priority
	...
	int pthread_mutex_init(pthread_mutex_t &mutex14, const pthread_mutexa//compile with: g++ -lpthread <sourcename> -o <executablename>
...
ttr_t &attr1);
	...
	// period, but the end of the first period and beginning of the next one. 
  	for (i = 0; i < NPERIODICTASKS; i++)
    	...
    	//Set again the global variable to zero because during the U computations they had been modified
	T1T2 = 0;
//compile with: g++ -lpthread <sourcename> -o <executablename>
...
{
  	// set task periods in nanoseconds
	periods[0]= 80000000;	//the first task has period 80 milliseconds
  	periods[1]= 100000000;	//the second task has period 100 milliseconds
  	periods[2]= 160000000;	//the third task has period 160 milliseconds
  	periods[3]= 200000000;	//the fourth task has period 200 milliseconds
	...
	printf ("\n");
  	for (i = 0; i < NTASKS; i++)
    	...
    	//before the end of the program, the mutexes and theirs attributes shall be destroyed    	
...
void task1_code()
{
	...
	double uno;
  	...
  	int pthread_mutex_lock(pthread_mutex_t &mutex14 );
...
void *task1( void *ptr)
{
	...
	pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cset);
  	...
  	for (i=0; i < 100; i++)
		...
		if ((1000000000*(time_now.tv_sec - next_arrival_time[0].tv_sec)
			       +(time_now.tv_nsec - next_arrival_time[0].tv_nsec)) > 0	
		) missed_deadlines[0]++;		
		...
		//Tried this instead of the lines before for missed deadline checks but doesn't work
		//if( !clock_nanosleep(CLOCK_REALTIME, TIMER_ABSTIME, &next_arrival_time[0], NULL)) missed_deadlines[0]++;	
	...
	double uno;
  	for (i = 0; i < OUTERLOOP; i++)
    	...
    	int pthread_mutex_lock(pthread_mutex_t &mutex12 );
  		...
  		//read from T1T2
  		double temp=T1T2;
  		//Shows how T1T2 increases in time (it stops at 100 because T1 runs for 100 times)
  		printf(" ");
  		printf("%f", temp);
  		printf(" ");
  	...
  	for (i=0; i < 100; i++)
    	{
      		...
      		struct timespec time_now;
		clock_gettime(CLOCK_REALTIME, &time_now);		
		if ((1000000000*(time_now.tv_sec - next_arrival_time[1].tv_sec)
			       +(time_now.tv_nsec - next_arrival_time[1].tv_nsec)) > 0	
		) missed_deadlines[1]++;
	...
	double uno;
  	for (i = 0; i < OUTERLOOP; i++)
    	...
    	int pthread_mutex_lock(pthread_mutex_t &mutex23 );
  		...
  		//read from T2T3
  		double temp=T2T3;
  		//Shows how T2T3 increases in time (it stops at 100 because T2 runs for 100 times)
  		printf(" ");
  		printf("%f", temp);
  		printf(" ");
  	...
  	for (i=0; i < 100; i++)
    	{
      		...
      		struct timespec time_now;
		clock_gettime(CLOCK_REALTIME, &time_now);		
		if ((1000000000*(time_now.tv_sec - next_arrival_time[2].tv_sec)
			       +(time_now.tv_nsec - next_arrival_time[2].tv_nsec)) > 0	
		) missed_deadlines[2]++;
	...
	double uno;
  	for (i = 0; i < OUTERLOOP; i++)
    	...
    	int pthread_mutex_lock(pthread_mutex_t &mutex14 );
  		...
  		//Shows how T1T4 increases in time (it stops at 100 because T1 runs for 100 times)
  		printf(" ");
  		printf("%f", temp);
  		printf(" ");
...
void *task4( void *ptr)
{
	...
	int i=0;
  	for (i=0; i < 100; i++)
		...
		struct timespec time_now;
		clock_gettime(CLOCK_REALTIME, &time_now);		
		if ((1000000000*(time_now.tv_sec - next_arrival_time[3].tv_sec)
			       +(time_now.tv_nsec - next_arrival_time[3].tv_nsec)) > 0	
		) missed_deadlines[3]++;
